#!/usr/bin/env bash
#
# step_4_smoothing_optional_dce.sh
#############################################################################
# Purpose : Optional smoothing of DCE brain masks and brain extracted images
...
# Usage   : bash step_4_smoothing_optional_dce.sh
# Input   : Subject directories containing dce/SWAN__brain_mask.nii.gz
#           and dce/corrected_dce_scan.nii.gz
...
#############################################################################
# Step: Smooth brain mask and generate smoothed brain extracted image - DCE
#############################################################################
echo "######## Starting DCE mask smoothing ########"

find "${SUBJECT_DIR}" -type d -mindepth 1 -maxdepth 1 | while IFS= read -r d; do
    echo "  Processing: $(basename $d)"

    brain_mask="$d"/dce/SWAN__brain_mask.nii.gz
    fixed_img="$d"/dce/corrected_dce_scan.nii.gz
    smooth_mask="$d"/dce/SWAN__brain_mask_smooth.nii.gz
    smooth_brain="$d"/dce/SWAN_brain_smooth.nii.gz
...
echo ""
echo "######## step_4_smoothing_optional_dce: Done ########"
echo "    Next: run step_4_smoothing_optional_rare.sh"
