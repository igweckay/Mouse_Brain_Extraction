#!/usr/bin/env bash
#
# step_3_batch_segmentation_dce.sh
#############################################################################
# Purpose : Perform brain extraction on all DCE subject scans
#           Registers the DCE template to each subject's native space,
#           applies the resulting transformation to the brain mask,
#           and multiplies the mask by the subject scan to extract the brain
# Usage   : bash step_3_batch_segmentation_dce.sh
# Input   : Subject directories containing dce/corrected_dce_scan.nii.gz
# Output  : Brain mask and brain extracted image in each subject's dce/ folder
# Next    : Run step_3_batch_segmentation_rare.sh
#############################################################################
...
moving_img=${TEMPLATE_DIR}/corrected_template_dce.nii.gz
atlas_label_file=${TEMPLATE_DIR}/brain_mask_template_dce.nii.gz

#############################################################################
# Step: Registration and brain extraction - DCE
#############################################################################
echo "######## Starting DCE brain extraction ########"

find "${SUBJECT_DIR}" -type d -mindepth 1 -maxdepth 1 | while IFS= read -r d; do
    echo "  Processing: $(basename $d)"

    fixed_img="$d"/dce/corrected_dce_scan.nii.gz
    output_dir="$d"/dce
...
echo ""
echo "######## step_3_batch_segmentation_dce: Done ########"
echo "    Next: run step_3_batch_segmentation_rare.sh"